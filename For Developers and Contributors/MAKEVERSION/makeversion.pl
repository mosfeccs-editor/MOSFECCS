#!/usr/bin/perl
#	makeversion.pl	Version 200310-1
...
# RESULT: a file MOSFECCS_v5_xxxxxx.html with the normal version (without SVG)
#                version and build as well as title are adjusted from xxxxxx-y in source file name
#         a file MOSFECCS_v5_xxxxxxcc.html with the compiled normal version (-> "Hauptdatei" in Moodle)
...
# Tests:  products generated from MOSFECCS_v6_SVG_210424-1.html (source) 
#         tested manually (MOSFECCS_V6_210424.html and MOSFECCS_v6_210424cc.html)
#         mostest_v5_210424.js with BJ_all_testMol.txt
#         msvg_210424.js with BP_W2020 and BP_S2019D.
#
#         B.Jaun 10 March 2020. 
...
my $msvgframe = $dir . "/specific_code/msvg_frame.txt";
my $msvgsvg = $dir . "/specific_code/msvg_SVG.txt";
my $msvgpms = $dir . "/specific_code/msvg_pms.txt";
...
my $mostest_tmp = '';
my $msvg = ''; # the final msvg_xxxxxx.js file
my $msvg_tmp = '';
...
$msvg = $dir . "/msvg_" . $version . ".js";
$msvg_tmp = $dir . "/msvg_tmp" . $version . ".js";
...
# MSVG Making the msvg_xxxxxx.html file
#========================================
# open the msvg_versionnumber.js file for writing
  open (OUT, ">$msvg_tmp") || die "Error: Could not open file $msvg_tmp for writing\n"; # overwrite or create
# open the mostest_frame.txt file
  open (IN, $msvgframe) || die "Error: template file $mostestframe not found!\n";
  while (<IN>) {
    print OUT;
  }
  close IN;
  ...
  open (OUT, ">>$msvg_tmp") || die "Error: Could not open $msvg_tmp file\n"; # append
  open (IN, $msvgsvg) || die "Error: template file $msvgsvg not found!\n";
  while (<IN>) {
    print OUT;
  }
  close IN;

# open mv_SVG HTML file and extract the SVG,COMMON parts.
	open (SRC, $source) || die "Error: could not open $source for reading\n";
	while (<SRC>) { 
	  print OUT if (/^\/\/START SVG/../^\/\/END SVG/);
	}
  ...
  open (IN, $msvgpms) || die "Error: template file $msvgpms not found!\n";
  while (<IN>) {
    print OUT;
  }
  close IN;
...
# open mv_SVG HTML file and extract the PMS part.
  ...
  # replace mv function calls by msvg function calls, uncomment //uc_msvg lines in parseSMILES main  
  open (IN, $msvg_tmp) || die "Error: could not open $msvg_tmp for reading\n";
  open (OUT, ">$msvg") ||  die "Error: Could not open file $msvg for writing \n";
  while ($line = <IN>) {
    if ($line =~ /\/\/ msvg_/) {
      $line = "// msvg_" . $version . ".js" . "\n";
    }
    if ($line =~ /const version/) {
      $line = '        const version = ' . '"' . "msvg_" . $version . '"' . "\n";
    }
    if ($line =~ /const build/) {
      $line = '        const build = ' . '"' . $build . '"' . "\n";
    }
    if ($line =~ /end of drawCanvas\(\) function/) {
      $line = "    } )(); // end of msvg() IFFE function" . "\n";
    }  
    $line =~ s/clearSelection\(/clearSelection_svg\(/g;
    $line =~ s/deleteAtom\(/deleteAtom_svg\(/g;
    $line =~ s/getboundrect\(/getboundrect_svg\(/g;
